...
tmosEvents HAL_ProcessEvent(tmosTaskID task_id, tmosEvents events)
{
    ...
    if(events & HAL_TINY_MACRO_OS_EVENT)
    {
        OS_UPDATE_TIMERS();
        OS_RUN_TASK(os_at_uart);
        return events ^ HAL_TINY_MACRO_OS_EVENT;
    }
...
void HAL_Init()
{
    ...
    tmos_start_reload_task(halTaskID, HAL_REG_INIT_EVENT, MS1_TO_SYSTEM_TIME(BLE_CALIBRATION_PERIOD)); // 添加校准任务，单次校准耗时小于10ms
#endif
    tmos_start_reload_task( halTaskID, HAL_TINY_MACRO_OS_EVENT, 1 );    // 添加一个测试任务
}
